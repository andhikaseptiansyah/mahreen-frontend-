...
const navbarStyles = `
  ...
  /* --- Mengatur Hover (Efek Teks Glow Emas Tanpa Background) --- */
  .navbar__link:hover,
  ...
  .navbar__ecosystem-button:focus-visible {
    ...
    text-shadow:
      ...
      0 0 16px rgba(215, 185, 130, 0.5),
      0 0 24px rgba(215, 185, 130, 0.2);
    box-shadow: none !important; 
  }

  .navbar__ecosystem-button:hover svg,
  .navbar__ecosystem-button:focus-visible svg {
    color: var(--navbar-gold) !important;
    filter: drop-shadow(0 0 6px rgba(215, 185, 130, 0.7));
  }
  ...
  /* Perbaikan: Menghilangkan kotak background pada menu dropdown aktif */
  ...
  return (
    <>
      ...
      <header className={`site-header${isScrolled ? " is-scrolled" : ""}`}>
        <nav className="navbar" aria-label="Navigasi utama Mahreen">
          {/* UBAH LINK HREF INI SESUAI HALAMAN NAVBAR-NYA (#/mahreen-studio, #/peduli-mahreen, #/internship, #/tanya-mahreen) */}
          <a
            className="navbar__brand"
            href="#/mahreen-studio"
            aria-label="Mahreen Beranda"
            ...
            <div className="navbar__mobile-navigation">
              <a
                className={`navbar__mobile-link${
                  isActiveRoute(currentPath, "#/mahreen-studio") ? " is-active" : ""
                }`}
                href="#/mahreen-studio"
                aria-current={
                  isActiveRoute(currentPath, "#/mahreen-studio") ? "page" : undefined
                }
                onClick={closeMobileMenu}
              >
                Home
